...
const copySidebar = document.querySelector('.copy-sidebar');
const sidebarToggle = document.querySelector('.copy-sidebar-toggle');
...
function calculate() {
  ...
  let ipBoxCoeff =
    parseFloat(document.getElementById("ipBoxCoeff").value) / 100;
  const healthContribLimit = 12900; // F16, Maksymalna składka zdrowotna do odliczenia podatek liniowy 
  let C16;
  if (income / 12 <= 6427.67) {
    C16 = 3779.52;
  } else {
    C16 = 0.049 * income;
  }
...
sidebarToggle.addEventListener('click', () => {
    copySidebar.classList.toggle('expanded');
});
...
function getFormattedValues() {
    ...
    const ipBoxCoeff = document.getElementById('ipBoxCoeff').value;
    ...
    // Add ryczałt values for checked rates
    const anyRyczaltVisible = Array.from(ryczaltCheckboxes).some(checkbox => checkbox.checked);
